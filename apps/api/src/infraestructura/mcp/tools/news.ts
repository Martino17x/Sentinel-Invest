...
// ============================================================
// news â€” Ãºltimas noticias TradingView headlines (+ Yahoo fallback)
// Thin wrapper â†’ analysisService.news â†’ {ok,message} es-AR,
...
export const newsTool: ToolDefinition = {
  name: "news",
  description:
    "Ãšltimas noticias de un instrumento (tÃ­tulo, fuente, tiempo, link). Usalo cuando pregunten novedades o por quÃ© se mueve un activo.",
  ...
  execute: async (ctx, rawArgs) => {
    ...
    if (res.status !== "ok" || !res.data) {
      ...
      if (res.status === "symbol_not_found" || detail.toLowerCase().includes("no encontrado")) {
        return { ok: false, message: `Noticias de ${args.symbol.toUpperCase()} no disponibles: SÃ­mbolo no encontrado${src}.` };
      }
    ...
    return {
      ok: true,
      message: `Noticias de ${args.symbol.toUpperCase()} â€” fuente: ${res.source} (${cachedNote}):\n${lines.join("\n")}`,
    };
  },
};
