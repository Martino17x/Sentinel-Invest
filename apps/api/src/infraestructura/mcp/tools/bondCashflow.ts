...
import { DISCLAIMER, fetchCashflow } from "../../../services/market/bonds/bondsQueries.js";
import type { ToolDefinition } from "../../../aplicacion/agente/types.js";

// ============================================================
// get_bond_cashflow â€” Flujo proyectado de la cartera de bonos
//
...
export const getBondCashflowTool: ToolDefinition = {
  ...
  description:
    "Flujo futuro de bonos de tu cartera: renta+amortizaciÃ³n por mes (CashflowMonth). Usa tu cuenta (ctx.account.id) â€” no acepta accountId del LLM. Filtros opcionales: symbol y monthsAhead 1..12. Paridad con GET /bonds/cashflow.",
  ...
  execute: async (ctx, rawArgs) => {
    ...
    // Aplicar monthsAhead si el servicio retornÃ³ 12 fijos: cortar por cutoff
    let filtered = months;
    if (monthsAhead < 12) {
      const from = new Date();
    ...
    // Filtrar por sÃ­mbolo si se pidiÃ³ â€” R4 spec scenario symbol
    if (symbolFilter) {
    ...
    if (filtered.length === 0) {
      ...
      return {
        ok: true,
        message: `Cashflow${symNote} vacÃ­o â€” sin pagos futuros en prÃ³ximos ${monthsAhead} meses (cuenta ${accountId.slice(0, 8)}â€¦). Generado ${generatedAt}\n${DISCLAIMER}`,
      };
    }

    // Orden cronolÃ³gico ya garantizado por projectCashflow (month ASC)
    ...
    return {
      ok: true,
      message: `Cashflow${symNote} (${filtered.length} meses, prÃ³ximos ${monthsAhead} meses, cuenta ${accountId.slice(0, 8)}â€¦) â€” generado ${generatedAt}\n${lines.join("\n")}\n${DISCLAIMER}`,
    };
  },
};
