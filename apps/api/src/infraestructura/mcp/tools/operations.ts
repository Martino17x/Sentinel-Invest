...
import type { Operation } from "../../../services/iol/types.js";
import type { ToolDefinition } from "../../../aplicacion/agente/types.js";

// ============================================================
// get_operations â€” IOL operations listing (P1 thin-wrapper)
...
function formatOperations(ops: Operation[], capped: boolean, total: number): string {
  ...
  const suffix = capped ? ` (mostrando ${ops.length} de ${total} â€” filtrÃ¡ por from/to/status/market)` : ` (total ${total})`;
...
export const getOperationsTool: ToolDefinition = {
  ...
  execute: async (ctx, rawArgs) => {
    ...
    // Do not fallback to portfolio history â€” only getOperations
    try {
      ...
      // Current IolProvider signature has no signal param â€” we race with abort.
      if (ctx.signal.aborted) {
        return { ok: false, message: `Operaciones: down â€” timeout 15s` };
      }
      ...
      return {
        ok: false,
        message: `Operaciones: error â€” ${err instanceof Error ? err.message : "Error al consultar operaciones"}`,
      };
    }
  },
};
