...
// ============================================================
// get_news_feed â€” feed GLOBAL de noticias sin sÃ­mbolo
// Thin-wrapper parity a GET /api/analysis/news/feed?limit.
// Llama directo a AnalysisService.newsFeed (GNews â†’ Finnhub â†’ TV),
// no fetch a propia API. Propaga ctx.signal, never throws,
// capa output (max 20, cap 10 lÃ­neas para contexto LLM).
// ============================================================
...
export const getNewsFeedTool: ToolDefinition = {
  name: "get_news_feed",
  description:
    "Feed global de noticias financieras (sin sÃ­mbolo). Parity a GET /api/analysis/news/feed. Cascade GNews â†’ Finnhub â†’ TradingView. Param limit 1-20 default 5. Usalo para panorama de mercado sin ticker especÃ­fico.",
  ...
  execute: async (ctx, rawArgs) => {
    ...
    try {
      if (ctx.signal.aborted) {
        return { ok: false, message: "News feed: down â€” timeout 15s" };
      }
      ...
      if (!items || items.length === 0) {
        return { ok: true, message: `News feed (limit ${limit}): sin noticias recientes â€” fuentes no devolvieron resultados.` };
      }
      ...
      const lines = slice.map((it, idx) => {
        ...
        const src = it.source ? ` â€” ${it.source}` : "";
        const when = ago ? ` (${ago})` : "";
      ...
      return {
        ok: true,
        message: `News feed â€” ${slice.length} noticias${suffix}:\n${lines.join("\n")}`,
      };
    } catch (err) {
      if (ctx.signal.aborted) {
        return { ok: false, message: "News feed: down â€” timeout 15s" };
      ...
      return {
        ok: false,
        message: `News feed: error â€” ${err instanceof Error ? err.message : "Error al consultar el feed"}`,
      };
    }
  },
};
