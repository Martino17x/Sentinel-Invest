...
import { addArtDays, artDateKeyFromUtc } from "../../../aplicacion/portafolio/art-time.js";
import type { ToolDefinition } from "../../../aplicacion/agente/types.js";

// ============================================================
// get_series â€” serie diaria + composiciÃ³n opcional
...
export const getSeriesTool: ToolDefinition = {
  name: "get_series",
  description:
    "Serie diaria de valor del portafolio por rango (portfolio_snapshots). Parity a GET /api/portfolio/series. Requiere from YYYY-MM-DD, to opcional inclusivo, includePositions opcional para traer composiciÃ³n por activo. Devuelve hasta 365 dÃ­as.",
  ...
  execute: async (ctx, rawArgs) => {
    ...
    try {
      if (ctx.signal.aborted) {
        return { ok: false, message: "Serie: down â€” timeout 15s" };
      }
      ...
      if (snapshots.length === 0) {
        return {
          ok: true,
          message: `Serie ${args.from} â†’ ${args.to ?? args.from}: sin snapshots en el rango (total 0).`,
        };
      }
      ...
      const dayLines = slice.map((s) => {
        ...
        return `- ${date} | total ${fmtMoney(Number(s.totalValue))} (USD ${Number(s.totalValueUsd).toFixed(2)}) | cash ${fmtMoney(Number(s.cashArs))} ARS / ${Number(s.cashUsd).toFixed(2)} USD | pos ${fmtMoney(Number(s.positionsValue))} | dÃ­a ${fmtPct(Number(s.dayChangePct))} | ${s.source}`;
      });
      ...
      if (args.includePositions && snapshots.length > 0) {
        ...
        if (totalPos === 0) {
          positionsBlock = "\nPosiciones: sin datos de composiciÃ³n en el rango.";
        } else {
      ...
      return {
        ok: true,
        message: `Serie ${args.from} â†’ ${args.to ?? args.from}${suffix}:\n${dayLines.join("\n")}${positionsBlock}`,
      };
    } catch (err) {
      if (ctx.signal.aborted) {
        return { ok: false, message: "Serie: down â€” timeout 15s" };
      ...
      return {
        ok: false,
        message: `Serie: error â€” ${err instanceof Error ? err.message : "Error al consultar la serie"}`,
      };
    }
  },
};
