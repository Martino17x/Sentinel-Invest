...
import { DISCLAIMER } from "../../../services/market/bonds/bondsQueries.js";
...
import type { BondAnalytics, BondSchedule } from "../../../services/market/bonds/types.js";

// ============================================================
// get_bond_ficha â€” Ficha tÃ©cnica por sÃ­mbolo
//
...
export const getBondFichaTool: ToolDefinition = {
  name: "get_bond_ficha",
  description:
    "Ficha tÃ©cnica de un bono: emisor, moneda, vencimiento, cupÃ³n, ley, denominaciÃ³n, ISIN, schedule, cuadro tÃ©cnico y paridad. Requiere sÃ­mbolo 2-12 A-Z0-9.",
  ...
  execute: async (ctx, rawArgs) => {
    ...
    try {
      const [fichaR, quoteR, maeR, schedR] = await Promise.all([
        ...
        provider.getQuote({ id: "", email: "" } as unknown as import("../../../services/iol/types.js").IolCredentials, symbol, "bcba").catch(() => null),
    ...
    const tirStr = tir != null ? `${(tir * 100).toFixed(2)}%` : "â€”";
    const mdStr = md != null ? md.toFixed(3) : "â€”";
    const durStr = duration != null ? duration.toFixed(3) : "â€”";
    const precioStr = dirtyPrice.toFixed(2);
    const paridadStr = cuadroTecnico.paridad != null ? `${cuadroTecnico.paridad.toFixed(2)}%` : "â€”";
    const cuponStr = couponRate != null ? `${(couponRate * 100).toFixed(2)}% ${frequency ? `${frequency}x/aÃ±o` : ""} ${dayCountPc}`.trim() : "â€”";
    const vencStr = finalSchedule.vencimiento ?? "â€”";
    const moneda = finalSchedule.moneda ?? "â€”";
    const staleNote = cerStale ? " (CER stale)" : "";

    return {
      ok: true,
      message:
        `Ficha ${symbol}${staleNote}: emisor ${cuadroTecnico.emisor ?? "â€”"} | moneda ${moneda} | vencimiento ${vencStr} | cupÃ³n ${cuponStr} | ley ${cuadroTecnico.ley ?? "â€”"} | ISIN ${cuadroTecnico.isin ?? "â€”"} | denominaciÃ³n mÃ­n ${cuadroTecnico.denominacionMinima ?? "â€”"} | precio ${precioStr} | TIR ${tirStr} | MD ${mdStr} | duration ${durStr} | paridad ${paridadStr} | VT ${cuadroTecnico.vt?.toFixed(2) ?? "â€”"} | VR ${cuadroTecnico.vr} | accrued ${accrued?.toFixed(2) ?? "â€”"} | schedule ${scheduleSource} | bid ${qbid ?? "â€”"} | ask ${qask ?? "â€”"} | low ${qlow ?? "â€”"} high ${qhigh ?? "â€”"} open ${qopen ?? "â€”"} close ${qclose ?? "â€”"}` +
        `\n${DISCLAIMER}`,
    };
  },
};
