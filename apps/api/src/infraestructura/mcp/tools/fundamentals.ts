...
// ============================================================
// fundamentals â€” fundamentales Yahoo-first (PER, EPS, betaâ€¦)
// Thin wrapper â†’ analysisService.getFundamentals â†’ {ok,message}
...
export const fundamentalsTool: ToolDefinition = {
  name: "fundamentals",
  description:
    "Fundamentales de una acciÃ³n/CEDEAR (PER, EPS, beta, margen, ROE, deuda/equity, dividend yield, market cap). Usalo cuando te pidan fundamentales o valuaciÃ³n.",
  ...
  execute: async (ctx, rawArgs) => {
    ...
    if (res.status !== "ok" || !res.data) {
      ...
      if (res.status === "symbol_not_found" || detail.toLowerCase().includes("no encontrado")) {
        return { ok: false, message: `Fundamentales de ${args.symbol.toUpperCase()} no disponibles: SÃ­mbolo no encontrado${src}. VerificÃ¡ el sÃ­mbolo y el mercado (bcba, nyse, nasdaq).` };
      }
      if (res.status === "rate_limited") {
        return { ok: false, message: `Fundamentales de ${args.symbol.toUpperCase()} no disponibles: Rate limit${src}. ProbÃ¡ de nuevo en unos segundos.` };
      }
    ...
    const lines = [
      `Fundamentales de ${args.symbol.toUpperCase()} â€” fuente: ${srcLabel} (${cachedNote}):`,
      `- PER: ${fmtNum(d.pe)}`,
      `- EPS: ${fmtNum(d.eps)}`,
      `- Beta: ${fmtNum(d.beta)}`,
