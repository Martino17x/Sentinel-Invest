...
import { isMarketHours } from "../../../dominio/analisis/isMarketHours.js";
import type { ToolDefinition } from "../../../aplicacion/agente/types.js";

// ============================================================
// get_bond_panel â€” Panel completo de bonos
//
// Thin-wrapper sobre fetchBondPanel (bondsQueries.ts) con
// filtrado por segment, orden y paginaciÃ³n local.
...
export const getBondPanelTool: ToolDefinition = {
  ...
  execute: async (ctx, rawArgs) => {
    ...
    if (segment && !VALID_SEGMENTS.includes(segment as (typeof VALID_SEGMENTS)[number])) {
      return { ok: false, message: `Segmento invÃ¡lido: ${segment}. VÃ¡lidos: ${VALID_SEGMENTS.join(", ")}` };
    }
    ...
    if (paginated.length === 0) {
      return {
        ok: true,
        message: `Panel bonos vacÃ­o (total ${total}, pÃ¡gina ${page}/${totalPages}, sort ${sort} ${order}${segment ? `, segment ${segment}` : ""}) â€” generado ${generatedAt}${closedNote}\n${DISCLAIMER}`,
      };
    }

    const lines = paginated.map((r) => {
      const tirStr = r.tir != null ? `${(r.tir * 100).toFixed(2)}%` : "â€”";
      const mdStr = r.md != null ? r.md.toFixed(3) : "â€”";
      const paridadStr = r.paridad != null ? `${r.paridad.toFixed(2)}%` : (r.cuadroTecnico?.paridad != null ? `${r.cuadroTecnico.paridad.toFixed(2)}%` : "â€”");
      const precioStr = r.precio != null ? r.precio.toFixed(2) : "â€”";
      const venc = r.vencimiento ?? r.schedule?.vencimiento ?? "â€”";
      const moneda = r.moneda ?? r.schedule?.moneda ?? "â€”";
    ...
    return {
      ok: true,
      message: `Panel bonos (${total} total, pÃ¡gina ${page}/${totalPages}, sort ${sort} ${order}${segment ? `, segment ${segment}` : ""}) â€” generado ${generatedAt}${closedNote}\n${lines.join("\n")}\n${DISCLAIMER}`,
    };
  },
};
