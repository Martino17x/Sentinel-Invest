...
import type { PortfolioSummary } from "../../../services/iol/types.js";
import type { ToolDefinition } from "../../../aplicacion/agente/types.js";
import { fmtArs, fmtPct, fmtQty, fmtUsd } from "./format.js";

// ============================================================
// get_portfolio â€” resumen completo de la cartera del usuario
//
...
function formatPortfolio(p: PortfolioSummary): string {
  ...
  lines.push(`Ganancia/pÃ©rdida total: ${fmtArs(p.gainLossArs)} (${fmtPct(p.dayChangePct)} el dÃ­a)`);

  if (p.positions.length === 0) {
    lines.push("Posiciones: (vacÃ­o â€” no hay activos en cartera)");
  } else {
  ...
  if (p.distribution.length > 0) {
    lines.push("DistribuciÃ³n: " + p.distribution.map((d) => `${d.label} ${d.pct.toFixed(1)}%`).join(", "));
  }
...
export const getPortfolioTool: ToolDefinition = {
  name: "get_portfolio",
  description:
    "Resumen completo de la cartera del usuario autenticado: totales ARS/USD, efectivo disponible, posiciones con cantidad, precio, valor y rendimiento, y distribuciÃ³n por activo. Sin argumentos.",
