...
import { parseIolMovements } from "../../../aplicacion/portafolio/iolMovementsParser.js";
import {
  fetchOperationsLike,
  getConfirmedMovements,
  getSnapshotPair,
} from "../../../aplicacion/portafolio/reconciliationData.js";
import { reconcileDay } from "../../../aplicacion/portafolio/reconciliation.js";

// ============================================================
// get_movements â€” cash_movements listing (P1 thin-wrapper)
  ...
  execute: async (ctx, rawArgs) => {
    ...
    try {
      if (ctx.signal.aborted) {
        return { ok: false, message: "Movimientos: down â€” timeout 15s" };
      }
      ...
      const suffix = capped ? ` (mostrando ${slice.length} de ${total} â€” filtrÃ¡ por from/to/source/status)` : ` (total ${total})`;
      ...
      return {
        ok: false,
        message: `Movimientos: error â€” ${err instanceof Error ? err.message : "Error al listar movimientos"}`,
      };
    }
  },
};
...
// ============================================================
// create_movement â€” POST /api/portfolio/movements
...
export const createMovementTool: ToolDefinition = {
  ...
  description:
    "Registra un movimiento manual de efectivo (aporte, retiro, dividendo, cauciÃ³n, ajuste). Parity a POST /api/portfolio/movements: inserta en cash_movements con source manual y status confirmed. Requiere date (YYYY-MM-DD), amount firmado, currency ARS|USD, type y description opcional.",
  inputSchema: z.object({
    date: dateSchema,
    amount: z.number({ message: "amount debe ser un nÃºmero (firmado)" }),
  ...
  execute: async (ctx, rawArgs) => {
    ...
    try {
      if (ctx.signal.aborted) {
        return { ok: false, message: "create_movement: down â€” timeout 15s" };
      }
      ...
      return {
        ok: false,
        message: `create_movement: error â€” ${err instanceof Error ? err.message : "Error al crear el movimiento"}`,
      };
    }
  },
};
...
// ============================================================
// patch_movement â€” PATCH /api/portfolio/movements/:id
// Solo pending â†’ confirmed/rejected (setea decided_at). Parity total.
// ============================================================

export const patchMovementTool: ToolDefinition = {
  name: "patch_movement",
  description:
    "Decide un movimiento pendiente (pending â†’ confirmed/rejected). Parity a PATCH /api/portfolio/movements/:id: solo pendientes pueden modificarse, setea decided_at. Requiere id y status confirmed|rejected, description opcional.",
  inputSchema: z.object({
  ...
  execute: async (ctx, rawArgs) => {
    ...
    try {
      if (ctx.signal.aborted) {
        return { ok: false, message: "patch_movement: down â€” timeout 15s" };
      }
      ...
      return {
        ok: false,
        message: `patch_movement: error â€” ${err instanceof Error ? err.message : "Error al actualizar el movimiento"}`,
      };
    }
  },
};
...
// ============================================================
// delete_movement â€” DELETE /api/portfolio/movements/:id
...
export const deleteMovementTool: ToolDefinition = {
  ...
  execute: async (ctx, rawArgs) => {
    ...
    try {
      if (ctx.signal.aborted) {
        return { ok: false, message: "delete_movement: down â€” timeout 15s" };
      }
      const [existing] = await db
        .select(movementColumns)
        .from(schema.cashMovements)
      ...
      return {
        ok: false,
        message: `delete_movement: error â€” ${err instanceof Error ? err.message : "Error al eliminar el movimiento"}`,
      };
    }
  },
};
...
// ============================================================
// import_movements_preview â€” POST /api/portfolio/movements/import
...
export const importMovementsPreviewTool: ToolDefinition = {
  ...
  description:
    "Preview del export IOL de movimientos (HTML crudo .xls). Parity a POST /api/portfolio/movements/import: parsea sin insertar, devuelve filas parseadas + flags de validaciÃ³n. El contenido del archivo se pasa como string htmlContent (no multipart).",
  inputSchema: z.object({
  ...
  execute: async (ctx, rawArgs) => {
    ...
    try {
      if (ctx.signal.aborted) {
        return { ok: false, message: "import_movements_preview: down â€” timeout 15s" };
      }
      const parsed = parseIolMovements(args.htmlContent);
      const total = parsed.summary.total;
      if (total === 0 && parsed.errors.length > 0) {
        return { ok: false, message: `Preview: sin filas â€” ${parsed.errors.join(" | ")}` };
      }
      const lines = parsed.movements.slice(0, MAX_PREVIEW_ROWS).map((mv) => {
        const flag = mv.valid ? "valid" : `invalid: ${mv.validationError ?? "error"}`;
        return `- fila ${mv.row} | nroMov ${mv.nroMov || "â€”"} | ${mv.liquidDate ?? "sin fecha"} | ${mv.monto} ${mv.currency} ${mv.tipo} | ${flag}${mv.tipoMov ? ` | ${mv.tipoMov}` : ""}`;
      });
      const capped = total > MAX_PREVIEW_ROWS ? ` (mostrando ${MAX_PREVIEW_ROWS} de ${total})` : "";
      const header = `Preview import IOL${capped}: total ${parsed.summary.total}, vÃ¡lidos ${parsed.summary.valid}, invÃ¡lidos ${parsed.summary.invalid} | por tipo ${JSON.stringify(parsed.summary.byType)}${parsed.errors.length ? ` | errores: ${parsed.errors.join(" | ")}` : ""}`;
      ...
      return {
        ok: false,
        message: `import_movements_preview: error â€” ${err instanceof Error ? err.message : "Error al parsear el export"}`,
      };
    }
  },
};
...
// ============================================================
// import_movements_confirm â€” POST /api/portfolio/movements/import/confirm
...
export const importMovementsConfirmTool: ToolDefinition = {
  ...
  execute: async (ctx, rawArgs) => {
    ...
    try {
      if (ctx.signal.aborted) {
        return { ok: false, message: "import_movements_confirm: down â€” timeout 15s" };
      }
      let imported = 0;
      let skipped = 0;
      ...
      return {
        ok: false,
        message: `import_movements_confirm: error â€” ${err instanceof Error ? err.message : "Error al confirmar el import"}`,
      };
    }
  },
};
...
// ============================================================
// reconcile â€” POST /api/portfolio/reconcile (preview, sin insertar)
// Parity a la ruta: snapshot pair (prev,hoy) + ops/movimientos del
// rango â†’ suggestions por moneda (ARS/USD) con deltaCash, expected,
...
export const reconcileTool: ToolDefinition = {
  ...
  execute: async (ctx, rawArgs) => {
    ...
    try {
      if (ctx.signal.aborted) {
        return { ok: false, message: "reconcile: down â€” timeout 15s" };
      }
      ...
      const lines = suggestions.map((r) => {
        const mv = r.movement
          ? ` â†’ sugerencia ${r.movement.type} ${r.movement.amount} ${r.movement.currency} | ${r.movement.message}${r.movement.suggestedType ? ` (sugerido: ${r.movement.suggestedType})` : ""}`
          : " â†’ sin movimiento sugerido";
      ...
      return {
        ok: true,
        message: `Reconcile ${today.date} (rango ${from} â†’ ${to}):\n${lines.join("\n")}`,
      };
    } catch (err) {
      if (ctx.signal.aborted) {
        return { ok: false, message: "reconcile: down â€” timeout 15s" };
      ...
      return {
        ok: false,
        message: `reconcile: error â€” ${err instanceof Error ? err.message : "Error al reconciliar"}`,
      };
    }
  },
};
