...
import { addArtDays, artStartOfDay } from "../../../aplicacion/portafolio/art-time.js";
import type { ToolDefinition } from "../../../aplicacion/agente/types.js";

// ============================================================
// get_portfolio_history â€” evoluciÃ³n REAL desde portfolio_snapshots
...
export const getPortfolioHistoryTool: ToolDefinition = {
  ...
  description:
    "EvoluciÃ³n histÃ³rica del valor del portafolio (portfolio_snapshots reales, nunca inventados). Parity a GET /api/portfolio/history. Params opcionales: days (1-365, default 90) o rango from/to YYYY-MM-DD. Devuelve hasta 365 puntos con total, cash y variaciÃ³n diaria.",
  ...
  execute: async (ctx, rawArgs) => {
    ...
    try {
      if (ctx.signal.aborted) {
        return { ok: false, message: "Historial: down â€” timeout 15s" };
      }

      const days = Math.min(args.days ?? 90, 365);

      // Rango en dÃ­as ART (PREREQ-1): mismo que route portfolio.ts
      ...
      if (total === 0) {
        return {
          ok: true,
          message: `Historial: sin snapshots en el rango solicitado (total 0). Los snapshots se generan al consultar el portfolio o vÃ­a cron diario.`,
        };
      }
      ...
      const lines = slice.map((s) => {
        ...
        return `- ${date} | total ${fmtMoney(totalValue)} | cash ${fmtMoney(cashArs)} ARS / USD ${cashUsd.toFixed(2)} | dÃ­a ${fmtPct(dayChangePct)} | ${src}`;
      });

      const suffix = capped
        ? ` (mostrando ${slice.length} de ${total} â€” filtrÃ¡ por from/to o days)`
        : ` (total ${total})`;
      return {
        ok: true,
        message: `Historial portafolio${suffix} â€” rango ${from.toISOString().slice(0, 10)} â†’ ${to.toISOString().slice(0, 10)}:\n${lines.join("\n")}`,
      };
    } catch (err) {
      if (ctx.signal.aborted) {
        return { ok: false, message: "Historial: down â€” timeout 15s" };
      ...
      return {
        ok: false,
        message: `Historial: error â€” ${err instanceof Error ? err.message : "Error al consultar el historial"}`,
      };
    }
  },
};
