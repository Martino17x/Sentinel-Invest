...
import { DISCLAIMER, fetchCurvePoints } from "../../../services/market/bonds/bondsQueries.js";
import { VALID_SEGMENTS } from "../../../services/market/bonds/curve.js";
import type { ToolDefinition } from "../../../aplicacion/agente/types.js";

// ============================================================
// get_bond_curve â€” Curva soberana por segmento o moneda
//
...
export const getBondCurveTool: ToolDefinition = {
  name: "get_bond_curve",
  description:
    "Curva soberana: puntos {ticker, tir, md, vencimiento, segmento} ordenados por vencimiento. FiltrÃ¡ por segment (USD-hard-dollar, BOPREAL, LECAP/BONCAP, CER) o currency (ARS, USD).",
  ...
  execute: async (ctx, rawArgs) => {
    ...
    // Resolver segmento: prioridad segment explÃ­cito > currency map > default
    let segment: string | undefined = args.segment;
    if (!segment && args.currency) {
      segment = args.currency === "USD" ? "USD-hard-dollar" : "CER";
    }
    // Si no se pasÃ³ nada, default USD-hard-dollar (mÃ¡s usado)
    ...
    if (sorted.length === 0) {
      return {
        ok: true,
        message: `Curva ${segment} vacÃ­a â€” sin puntos disponibles. Generado ${generatedAt}\n${DISCLAIMER}`,
      };
    }
    ...
    return {
      ok: true,
      message: `Curva ${segment} (${sorted.length} puntos) â€” generado ${generatedAt}\n${lines.join("\n")}\n${DISCLAIMER}`,
    };
  },
};
