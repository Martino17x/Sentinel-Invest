/**
 * RadarOrchestrator — orquestador on-demand + SWR con doble cache.
 *
 * Extraído de `services/market/radar.ts` (God Object 501l) sin modificar
 * ese archivo aún (fase 3 lo dejará como shim). Preserva verbatim el flujo:
 *
 *  1. BYMA postPanel("cedears") vía MarketDataPort.getPanel (único fetch)
 *  2. Join con ratios CEDEAR (RATIO_MAP) — filtra lastPrice>0 y ratio existente;
 *     USD C/D → ccl:null (excluido de promedio) vía isUsdSettlementVariant
 *  3a. Híbrido BYMA: si existe par ARS/USD para mismo base → ccl = ARS / USD
 *      (ratio se cancela). Preferido 100% BYMA sin Yahoo.
 *  3b. Fallback Yahoo: bulk fetchChart(yahooSymbol,"1d") con p-limit(6) + allSettled
 *      y ccl = ARS * ratio / USD
 *  4. calcCcl per row → calcPromedio (mediana) → calcSpread
 *  5. SwrCache<CclResponse> + SwrCache<RadarBase> TTL 15min, stale-serve + dedup
 *
 * Inyección por ctor (testeable sin mock global):
 *   - marketData: MarketDataPort (QuoteService)
 *   - fetchChart: (symbol, range, signal?) => Promise<ChartResult>  (yahoo.fetchChart)
 *   - isMarketHours: () => boolean  (isMarketHours(new Date()) curried)
 *
 * Sin duplicar lógica C/D: delega a `isUsdSettlementVariant` y `getBaseSymbol`
 * de @sentinel/domain. `getBase` local reusa `getBaseSymbol`.
 */
...
import type { ChartResult } from "../../services/market/yahoo.js";

// ---------------------------------------------------------------------------
// Types (re-exportados para shim y consumers)
// ---------------------------------------------------------------------------
...
/** Raw dataset cache (full unpaginated items) */
interface RadarBase {
  generatedAt: string;
  cclPromedio: number | null;
  disclaimer: string;
...
export type MarketDataPortLike = Pick<MarketDataPort, "getPanel">;
...
// ---------------------------------------------------------------------------
// Helpers (delegan a domain, no duplican lógica)
// ---------------------------------------------------------------------------

function isUsdVariant(symbol: string, currency: string): boolean {
  return isUsdSettlementVariant(symbol, currency);
}

function getBase(sym: string): string {
  return getBaseSymbol(sym);
}
...
export class RadarOrchestrator {
  private readonly baseCache = new SwrCache<RadarBase>(RADAR_TTL_MS);
  private readonly radarCache = new SwrCache<CclResponse>(RADAR_TTL_MS);
  private readonly inFlight = new Map<string, Promise<RadarBase>>();
  private readonly bgInFlight = new Map<string, Promise<void>>();
  private readonly limit = pLimit(6);

  constructor(
    private readonly marketData: MarketDataPortLike,
    private readonly fetchChart: FetchChart,
    private readonly isMarketHours: () => boolean,
  ) {}

  /** Exposición para shim futuro y tests (lectura) */
  get _baseCache(): SwrCache<RadarBase> {
    return this.baseCache;
  }
  get _radarCache(): SwrCache<CclResponse> {
    return this.radarCache;
  }
  /** Aliases públicos esperados por la fachada shim (radarCache / baseCache) */
  get radarCachePublic(): SwrCache<CclResponse> {
    return this.radarCache;
  }
  get baseCachePublic(): SwrCache<RadarBase> {
    return this.baseCache;
  }

  // Compat: la fachada shim accederá vía orchestrator["radarCache"] / ["baseCache"] (private).
  // Mantener campos con nombres exactos para ese acceso bracket.
  // También exponer via getter si el shim usa property access directo.

  private async fetchRadarBase(signal?: AbortSignal): Promise<RadarBase> {
    ...
    const panel = await this.marketData.getPanel(
      { id: "", email: "" } as never,
      "bcba",
      "cedear",
      1,
    ...
    const chartResults = await Promise.allSettled(
      yahooNeededQuotes.map((q) =>
        this.limit(async () => {
          const res = await this.fetchChart(q.yahooSymbol, "1d", signal);
          return { q, res };
        }),
      ),
    );
    ...
    const isMarketClosed = !this.isMarketHours();
  ...
  private refreshInBackground(signal?: AbortSignal): void {
    const key = CACHE_KEY;
    if (this.inFlight.has(key) || this.bgInFlight.has(key)) return;
    const p = this.fetchRadarBase(signal)
      .then((base) => {
        // No pisar cache válido con snapshot vacío fuera de horario
        const existing = this.baseCache.getEntry(key);
        ...
        const defaultResponse = applyFilterSortPaginate(
          base.allItems,
          { page: 1, limit: 50, sort: "spread" },
          base,
        );
        this.radarCache.set(key, defaultResponse);
      })
      .catch(() => {
        // keep stale
      })
      .finally(() => this.bgInFlight.delete(key));
    this.bgInFlight.set(key, p);
  }
  ...
  getRadar = async (opts: {
    ...
    const entry = this.baseCache.getEntry(CACHE_KEY);

    if (entry) {
      if (this.baseCache.isFresh(entry)) {
        return applyFilterSortPaginate(entry.data.allItems, { q, page, limit, sort, source }, entry.data);
      }
      // Stale-while-revalidate: serve stale immediately + refresh in background
      this.refreshInBackground(opts.signal);
      return applyFilterSortPaginate(entry.data.allItems, { q, page, limit, sort, source }, entry.data);
    }

    // Cache miss — check dedup
    const existing = this.inFlight.get(CACHE_KEY);
    if (existing) {
      const base = await existing;
      return applyFilterSortPaginate(base.allItems, { q, page, limit, sort, source }, base);
    }

    const promise = this.fetchRadarBase(opts.signal);
    this.inFlight.set(CACHE_KEY, promise);
    try {
      ...
      if (base.allItems.length === 0) {
        const staleEntry = this.baseCache.getEntry(CACHE_KEY);
        if (staleEntry && staleEntry.data.allItems.length > 0) {
          return applyFilterSortPaginate(
            staleEntry.data.allItems,
            { q, page, limit, sort, source },
            staleEntry.data,
          );
        }
        const legacyStale = this.radarCache.getEntry(CACHE_KEY);
        if (legacyStale && legacyStale.data.items.length > 0) {
          return legacyStale.data;
        }
      }
      this.baseCache.set(CACHE_KEY, base);
      const defaultResponse = applyFilterSortPaginate(
        base.allItems,
        { page: 1, limit: 50, sort: "spread" },
        base,
      );
      this.radarCache.set(CACHE_KEY, defaultResponse);
      return applyFilterSortPaginate(base.allItems, { q, page, limit, sort, source }, base);
    } catch (err) {
      ...
      this.inFlight.delete(CACHE_KEY);
    }
  };

  /** Only for tests: clear caches + inFlight (doble cache + bg dedup) */
  resetRadarCacheForTests(): void {
    this.baseCache.resetForTests();
    this.radarCache.resetForTests();
    this.inFlight.clear();
    this.bgInFlight.clear();
  }

  /** Alias requerido por spec D4 (resetForTests) */
  resetForTests(): void {
    this.resetRadarCacheForTests();
  }

  // Expose internal helpers for unit tests if needed (no romper encapsulado)
  // sortRows / applyFilterSortPaginate permanecen como funciones puras externas
}

// Re-export pure helpers for tests that import from orchestrator module
export { sortRows, applyFilterSortPaginate };

// Legacy top-level exports for shim compatibility (instancia singleton se crea en shim)
export const CACHE_KEY_EXPORT = CACHE_KEY;
export { RADAR_TTL_MS };
