...
/** Factory per-request: construye el McpServer con el scope + categorías de la key */
function makeFactory(): McpServerFactory {
  return (ctx) => {
    ...
    return createSentinelMcpServer({
      ...
      scope,
      enabledCategories: extra?.enabledCategories ?? null,
      clientName: "mcp:http",
    });
  };
}
      ...
      const response = await mcpHandler.fetch(expressReqToFetch(req), {
        authInfo: req.mcp
          ? {
              ...
              extra: { userId: req.mcp.userId, apiKeyId: req.mcp.apiKeyId, enabledCategories: req.mcp.enabledCategories },
            }
          : undefined,
        parsedBody: req.body,
      });
