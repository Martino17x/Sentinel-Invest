...
export function toZod4(schema: zod3.ZodTypeAny): z4.ZodTypeAny {
  ...
  switch (def.typeName) {
    ...
    case "ZodNullable":
      return toZod4(def.innerType as zod3.ZodTypeAny).nullable();
    case "ZodNull":
      return z4.null();
    case "ZodUnion": {
      const options = def.options as zod3.ZodTypeAny[] | undefined;
      if (!options || options.length === 0) throw new Error("ZodUnion sin options");
      return z4.union(options.map(toZod4) as [z4.ZodTypeAny, ...z4.ZodTypeAny[]]);
    }
    case "ZodDefault":
