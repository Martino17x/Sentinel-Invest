// ============================================================
// bondAnalyticsSnapshot.ts â€” Snapshot diario de analytics+curvas
//
// Corre lun-vie 17:10 ART (despuÃ©s del cierre BYMA 17:00, antes del
...
// routes/bonds.ts.
//
// DiseÃ±o (replica quotesSnapshot.ts):
// - Deps inyectables â†’ testeable con fakes.
...
async function defaultFetchAnalytics(): Promise<BondAnalytics[]> {
  ...
  // Para LECAP/BONCAP/CER, MAE no expone; se cubrirÃ¡ en Fase local
  // (por ahora snapshot guarda lo que MAE tiene; curve.ts agruparÃ¡).
...
export async function runBondAnalyticsSnapshot(
  ...
  try {
    ...
    if (!analytics || analytics.length === 0) {
      deps.log.warn("âš ï¸ bond-analytics-snapshot: sin analytics (MAE vacÃ­o o BYMA caÃ­do) â€” no se guarda snapshot");
    ...
    // Persistir como bonos: market=bonds, assetType=bonds (Ãºnico panel por ahora)
    ...
    deps.log.log(
      `ðŸ“¸ bond-analytics-snapshot: ${analytics.length} analytics, ${totalCurvePoints} puntos de curva guardados`,
    );
    ...
    deps.log.warn(`âš ï¸ bond-analytics-snapshot: ${message}`);
    return { ok: false, analyticsCount: 0, curvesCount: 0, error: message };
  }
}

/**
 * Helper para routes: lee Ãºltimo snapshot (stale fallback).
 * Reutiliza art-time cutoff de 7 dÃ­as.
 */
export async function getLatestBondAnalyticsSnapshot(
  ...
  const { artDateKeyFromUtc } = await import("../portafolio/art-time.js");
