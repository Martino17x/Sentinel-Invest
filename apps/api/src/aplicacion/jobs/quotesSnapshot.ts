// ============================================================
// QUOTES SNAPSHOT JOB â€” captura del panel BYMA al cierre
//
// Corre lun-vie 17:05 ART (despuÃ©s del cierre 17:00). Itera las
// combinaciones de (market, assetType) que realmente muestran
// datos en Cotizaciones y persiste cada panel en quotes_snapshots.
//
// DiseÃ±o (D3 del scheduler):
// - Dependencies inyectables â†’ testeable con fakes.
// - Un panel que falla se loguea y el job continÃºa con el resto.
...
import { QuoteService } from "../cotizaciones/QuoteService.js";
 ...
 * Por panel: fetchPanel â†’ saveSnapshot. Un fallo no detiene el resto.
 */
export async function runQuotesSnapshots(
  ...
  for (const { market, assetType } of QUOTES_PANELS) {
    try {
      ...
      // Solo persistir si hay datos (si BYMA estÃ¡ caÃ­do totalmente, no pisar snapshot vÃ¡lido con array vacÃ­o)
      if (quotes.length === 0) {
        deps.log.warn(`âš ï¸ quotes-snapshot [${market}/${assetType}]: panel vacÃ­o â€” no se guarda snapshot`);
        outcomes.push({ market, assetType, ok: false, quotesCount: 0, error: "panel vacÃ­o" });
        continue;
      }
      const payload = {
      ...
      deps.log.log(`ðŸ“¸ quotes-snapshot [${market}/${assetType}]: ${quotes.length} instrumentos guardados`);
      outcomes.push({ market, assetType, ok: true, quotesCount: quotes.length });
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      deps.log.warn(`âš ï¸ quotes-snapshot [${market}/${assetType}]: ${message}`);
