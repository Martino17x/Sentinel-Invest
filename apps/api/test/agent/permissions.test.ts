...
test("toolsVisibleForScope: read oculta los tools de trading, trade los lista", () => {
  ...
  // DOMAIN_TOOLS = 37 total; 4 con proposeOnly (place_order, cancel_order, subscribe_fci, rescue_fci)
  // read ve 33 (37-4), trade ve 37. Estos números fallarán si se agregan tools sin actualizar el test:
  // preferir aserción derivada + hardcode como guardrail documental.
  assert.equal(agentRegistry.list().length, 37);
  assert.equal(trade.length, 37);
  assert.equal(read.length, 33);
