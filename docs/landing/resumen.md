...
## Qué es
**Sentinel** es una aplicación web de **control de inversiones para cuentas de IOL (InvertirOnline), Argentina**. Se conecta a la cuenta del usuario en **modo solo lectura** — nunca ejecuta órdenes — y centraliza: valorización de cartera, cotizaciones AR/US en tiempo real, análisis técnico de acciones, historial de operaciones, reportes mensuales, dólar del día e integración con **agentes de IA vía MCP**. Tagline real: *"Tu cartera de inversiones, controlada"*.
...
- **Personas que valoran seguridad y transparencia:** cifrado AES-256, "nunca ejecuta órdenes", desconexión inmediata.

## Propuesta de valor
1. **Control total, cero riesgo:** solo lectura garantizado; credenciales cifradas AES-256 y eliminadas al desconectar.
...
6. **Tu agente de IA, conectado:** server MCP local con tools de lectura; `place_order` solo con confirmación humana (próxima versión).
...
- **Confiable y transparente:** "nunca ejecuta órdenes", "tus credenciales están cifradas con AES-256", "excluye aportes", "pesos renormalizados".
